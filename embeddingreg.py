...
class embedding(object):
        ...
        if self.weights_type == 'image_similarity':
            ...
            if self.is_conv:
                ...
                self.encoder = torch.nn.Sequential(
                    torch.nn.Linear(28*28, 300), # TODO: change input shape to match the image dims.
                    torch.nn.ReLU(),
        ...
        if current_task-1 not in self.tasks:
            ...
            with torch.no_grad():
                ...
                if self.supervised:
                    ...
                    self.embeddings_images[current_task-1] = [y1.cpu(), y2.cpu()]
                    ...
                    if len(self.embeddings) == 0:
                        ...
                        self.embeddings_images[0] = [y1.cpu(), y2.cpu()]
    ...
    def embedding_drive(self, current_batch):
        ...
        for t in self.embeddings:
            ...
            for i in idx:
                img_y1, img_y2 = self.embeddings_images[t][i].unsqueeze(0).to(self.device)
                embeddings = self.embeddings[t][i].unsqueeze(0)

                new_embeddings_y1 = self.model.embedding(img_y1)
                new_embeddings_y2 = self.model.embedding(img_y2)
                new_embeddings = torch.cat((new_embeddings_y1, new_embeddings_y2), 0)
                # if self.external_embedding_layer is not None:
                ...
                if self.weights_type is not None:
                    if self.weights_type == 'distance':
                        ...
                        current_images_y1 = self.encoder(current_batch[0])
                        current_images_y2 = self.encoder(current_batch[1])
                        old_images_y1 = self.encoder(img_y1)
                        old_images_y2 = self.encoder(img_y2)
                        old_images = torch.cat((old_images_y1, old_images_y2), 0)
                        current_images = torch.cat((current_images_y1, current_images_y2), 0)
